...
function getNextFriday(date) {
  ...
  const fridayIndex = days.findIndex((el) => el === 'Friday');
  let difference =
    dayIndex < fridayIndex
      ? fridayIndex - dayIndex
      : fridayIndex + (days.length - dayIndex);
  if (difference === 0) difference = 7;

  const dateMilliseconds = date.getTime();
  const diffMilliseconds = difference * 24 * 60 * 60 * 1000;
  const newDate = new Date(dateMilliseconds + diffMilliseconds);
  return newDate;
}
...
function isDateInPeriod(date, period) {
  const dateMl = new Date(date).getTime();
  const startMl = new Date(period.start).getTime();
  const endMl = new Date(period.end).getTime();
  return dateMl >= startMl && dateMl <= endMl;
}
...
function formatDate(date) {
  ...
  const pmAm = Number(date.split('T')[1].slice(0, 2)) >= 12 ? 'PM' : 'AM';
  return `${month}/${day}/${year}, ${hoursString}:${minutes}:${seconds} ${pmAm}`;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const [day1, month1, year1] = period.start.split('-');
  const startDate = new Date(year1, month1 - 1, day1).getTime();
  const [day2, month2, year2] = period.end.split('-');
  const endDate = new Date(year2, month2 - 1, day2).getTime();
  const days = [];
  ...
  days.forEach((day) => {
    if (countOff > 0) {
      res.push(null);
      countOff -= 1;
      if (countOff === 0) countWork = countWorkDays;
    } else if (countWork > 0) {
      const d = String(new Date(day).getDate()).padStart(2, '0');
      const m = String(new Date(day).getMonth() + 1).padStart(2, '0');
      const y = new Date(day).getFullYear();

      res.push(`${d}-${m}-${y}`);
      countWork -= 1;
      if (countWork === 0) countOff = countOffDays;
    }
  });
